...
`fsutil usn deletejournal /D C:` removes the journal outright. That is not something normal operation or legitimate administration does, and the absence of a journal on a volume where one should exist is a finding in its own right — the same reasoning as [Prefetch deletion](#/lesson/l2-01-prefetch).
...
- **Mass `FILE_DELETE` sequences** consistent with staging cleanup or [anti-forensic wiping](#/lesson/l2-15-vss-deletion-ransomware-precursor).
