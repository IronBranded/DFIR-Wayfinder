...
**1. The timestamp is the file's last-modified time, not an execution time.** ShimCache stores the file's `$STANDARD_INFORMATION` modified timestamp — the same value [timestomping manipulates](#/lesson/l2-02-mft-timestomping). It tells you when the file was last written, not when it ran, and it inherits every weakness of `$SI` timestamps.
...
- **An entry near the top of the ordering** (recent) for a suspicious path, corroborated against [Prefetch](#/lesson/l2-01-prefetch) or [Amcache](#/lesson/l2-04-amcache).
- **A modified timestamp inconsistent with the file's other MACB values** — potential timestomping, cross-checked in [the MFT lesson](#/lesson/l2-02-mft-timestomping).
