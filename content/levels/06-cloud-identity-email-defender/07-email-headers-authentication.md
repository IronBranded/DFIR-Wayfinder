...
Obtain the **original message**, not a forward — forwarding replaces the headers with the forwarder's own, destroying exactly the evidence you need. Request it as an attachment (`.eml`/`.msg`) or pull it from the mailbox directly. This is also the first step in [the phishing playbook](#/lesson/l8-08-playbook-phishing).
