...
This lesson is evidence-source content supporting detection broadly rather than mapping to one technique. Deliberate tampering with real-time protection or exclusions maps to [Impair Defenses: Disable or Modify Tools (T1562.001)](https://attack.mitre.org/techniques/T1562/001/) — the same technique the [PowerShell malicious-cmdlet-patterns lesson](#/lesson/l3-04-powershell-malicious-patterns) covers from the attacker-command side.

> [!TIP]
> [The next lesson](#/lesson/l6-19-attack-surface-reduction) covers the rule-based layer sitting on top of everything here — Attack Surface Reduction — and where Tamper Protection fits into stopping the tampering pattern flagged above.
