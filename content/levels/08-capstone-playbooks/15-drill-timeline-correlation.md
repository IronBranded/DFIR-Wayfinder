...
The corrected order tells a story the raw data did not: Word spawned PowerShell, which spawned a masqueraded `svchost.exe` from `C:\Users\Public`, which **then** fetched the payload at 09:02:37 — 31 seconds later, entirely consistent with a [download cradle](#/lesson/l3-04-powershell-malicious-patterns) executing.

The 96-byte POST at 09:07:55 is check-in traffic, not exfiltration — small, outbound, following successful privilege escalation. That is a [beacon](#/lesson/l7-02-proxy-firewall-triage) byte-ratio profile.
