...
> [!IMPORTANT]
> MFA was enforced and worked correctly. She authenticated properly. No password was stolen, because the attack never needed one — this is [the illicit consent grant](#/lesson/l3-14-oauth-consent-grants), and MFA is not a control against it. The `offline_access` scope granted a refresh token, and the attacker had durable mailbox access from that moment.
...
The only artifact generated is `MailItemsAccessed` — which is exactly why [the BEC playbook](#/lesson/l8-03-playbook-bec) treats it as the operation that separates "an account was accessed" from a defensible statement about exposure.

## Week 4 — Establishing position

Three actions in one session:

1. A [hidden inbox rule](#/lesson/l3-16-mailbox-forwarding-rules) created via EWS, invisible in Outlook: messages containing `wire`, `remittance`, or `payment instructions` move to **RSS Feeds** and are marked read.
