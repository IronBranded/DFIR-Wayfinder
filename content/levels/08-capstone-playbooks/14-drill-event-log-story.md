...
**11:08:47 — 7045.** The [service installation](#/lesson/l3-07-windows-services) from the registry drill, now with a timestamp. 7045 is in the **System** log and requires no audit policy — which is why it survives when Security auditing is patchy.

**11:09:12 — 4648.** [Explicit credentials](#/lesson/l2-09-event-log-key-ids) supplied, targeting `Administrator`. The attacker has obtained that credential and is using it against a remote target.
...
It is not gone from everything. [Log recovery](#/lesson/l7-06-log-artifact-recovery) applies: the System log still has 7045 and was not cleared; a SIEM or WEF collector may hold shipped copies; records may be carvable from unallocated space; and none of the [filesystem or registry artifacts](#/lesson/l2-03-usn-journal) were touched at all.
