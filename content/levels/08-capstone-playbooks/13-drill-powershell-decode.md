...
The step people skip is `Unicode.GetString`. Base64-decoding alone yields text with a null byte between every character — that is [UTF-16LE showing through](#/lesson/l3-02-powershell-obfuscation), not corruption.
...
Together these form the [download cradle](#/lesson/l3-04-powershell-malicious-patterns): fetch and execute in one expression. There is no file for antivirus to scan, no [Prefetch](#/lesson/l2-01-prefetch) entry for the payload, no [Amcache](#/lesson/l2-04-amcache) hash. This is what "fileless" means concretely.

**The destination is a raw IP over plain HTTP** — no domain, no TLS. Convenient for the attacker, and convenient for you: it is directly actionable as an indicator and would appear in [proxy or NetFlow data](#/lesson/l7-03-netflow-analysis).
...
Individually ordinary — `-nop` is close to best practice for unattended automation. Clustered, on a process spawned by Word, they are the [pattern](#/lesson/l3-04-powershell-malicious-patterns).
...
Recovery options, in order: [Script Block Logging (4104)](#/lesson/l3-01-powershell-logging) captured it post-parse if enabled; [memory](#/lesson/l7-08-memory-based-file-recovery) may still hold it if the host has not rebooted; proxy logs may have the response body.
