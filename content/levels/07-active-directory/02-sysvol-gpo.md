...
This is the payoff for the [ACL abuse](#/lesson/l7-14-acl-delegation-abuse) covered later in this level: `GenericWrite` or `WriteDacl` on a single, obscure, unmonitored GPO can be worth more than membership in a privileged group, and looks far less alarming in an access review.
...
> [!TIP]
> If a GPO change has no change record and the Security log doesn't reach far enough back to confirm when it happened, [replication metadata](#/lesson/l7-03-replication-metadata) — the next lesson — can often answer that question from AD itself.
