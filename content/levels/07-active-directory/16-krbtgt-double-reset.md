...
> [!IMPORTANT]
> This is the same "reset twice" principle as the [hybrid identity runbook](#/lesson/l2-04-hybrid-runbook), but for a completely different underlying reason. There, the second reset addresses password-hash sync back to on-premises AD. Here, it's about flushing AD's own two-hash history buffer. Same instruction, different mechanism — worth keeping straight.
