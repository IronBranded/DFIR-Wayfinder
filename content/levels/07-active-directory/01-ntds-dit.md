...
**`secretsdump.py`** (from Impacket) can operate remotely against a DC, or parse an already-obtained `NTDS.dit` plus SYSTEM hive offline. Used remotely, it overlaps heavily with the [DCSync technique](#/lesson/l7-06-dcsync-detection) covered separately — the difference being whether credentials are pulled from the file or requested through replication.
