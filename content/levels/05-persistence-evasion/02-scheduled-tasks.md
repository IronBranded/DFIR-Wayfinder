...
> [!TIP]
> The legacy `at.exe` command creates jobs in a different place entirely — `C:\Windows\Tasks\*.job`, a binary format rather than XML, with no entry in the Task Scheduler tree. `at` is deprecated and absent from modern Windows, but `.job` files persist on long-lived systems and in older images, and a triage that only walks `System32\Tasks\` will not see them.
...
- **An action invoking `powershell.exe`, `mshta.exe`, `rundll32.exe`, `regsvr32.exe`, or `cmd.exe`** — see [LOLBins](#/lesson/l5-14-lolbins).
