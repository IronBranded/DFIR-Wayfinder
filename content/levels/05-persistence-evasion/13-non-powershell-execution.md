...
Beyond the enumeration/discovery use covered in [Level 2](#/lesson/l7-04-discovery), WMI can execute commands directly — locally via `wmic process call create`, or remotely against another host entirely via the same interface, without ever touching PowerShell's remoting stack (WinRM) or its logging.

```
wmic /node:target-host process call create "cmd.exe /c whoami"
```

This is genuinely remote code execution over a completely legitimate management protocol, and it's also how [WMI Event Subscription persistence](#/lesson/l5-04-wmi-subscriptions) gets established in the first place — the same interface serving both initial execution and long-term persistence.
...
- **A cluster of WMI activity immediately following the discovery-command pattern** from [Level 2](#/lesson/l7-04-discovery) — an attacker who just enumerated the environment, now using WMI to actually move through it.
