...
The same inter-arrival analysis from [DNS](#/lesson/l6-01-dns-analysis) applies to connection logs, with the advantage that byte counts are available too: a beacon shows both **regular timing** and **consistent, small transfer sizes**. Two weak signals that correlate become one strong one.
