...
[Event ID 1102](#/lesson/l2-09-event-log-key-ids) is written to the Security log **immediately after** it is cleared — so the first entry in the newly-empty log is a record of the clearing, including the account that did it. `104` does the same for other logs.
...
**5. Volume Shadow Copies.** A shadow copy predating the clear contains the intact `.evtx` file. See [VSS recovery](#/lesson/l7-07-vss-recovery).

**6. Memory.** Recently-written records may still be in the Event Log service's working set, recoverable from a memory image — another reason [memory acquisition comes first](#/lesson/l5-01-acquisition).
...
- [`$MFT` and `$FN` timestamps](#/lesson/l2-02-mft-timestomping)
- [The USN journal](#/lesson/l2-03-usn-journal)
- [Prefetch](#/lesson/l2-01-prefetch), [Amcache](#/lesson/l2-04-amcache), [ShimCache](#/lesson/l2-05-shimcache)
- [Registry key `LastWriteTime` values](#/lesson/l2-06-registry-hives)
- [ShellBags](#/lesson/l2-07-shellbags) and [UserAssist](#/lesson/l2-08-userassist)
- [AD replication metadata](#/lesson/l4-03-replication-metadata)
