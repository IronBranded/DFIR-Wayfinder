...
Enable the Windows DNS Server **Analytical** log (not the deprecated debug log) on internal resolvers. Deploy [Sysmon](#/lesson/l1-11-sysmon-deployment) with Event ID 22 enabled for process attribution. For beaconing analysis, extract inter-arrival times per (host, domain) pair and look for low standard deviation. [RITA](#/lesson/l7-09-c2-framework-fingerprinting) and similar tooling automate the statistical side.
