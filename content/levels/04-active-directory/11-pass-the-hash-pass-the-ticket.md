...
NTLM authentication never actually needs your cleartext password — it needs the **NTLM hash** of that password. Pass the Hash exploits this directly: an attacker who has captured an NTLM hash (via [LSASS memory access](#/lesson/l5-08-lsass-memory-analysis), a credential dump, or an intercepted authentication) can present that hash to authenticate as the user, on any system that accepts NTLM, without ever knowing or cracking the underlying password.
