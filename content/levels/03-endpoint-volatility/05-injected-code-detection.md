...
This is the memory-forensics-side confirmation of a pattern this academy has approached from several other directions already: a download cradle from [Level 3](#/lesson/l3-16-powershell-malicious-patterns) that reflectively loads an assembly never writes a file [Level 2's LOLBins content](#/lesson/l5-14-lolbins) would catch — but it has to land *somewhere* in memory to actually run, and that somewhere is exactly the private-executable-with-content signature `malfind` is built to catch.
...
- **A `malfind` hit in a process whose command line or parent process** (from [the process-analysis lesson](#/lesson/l3-02-volatility-process-analysis)) **already looked suspicious** — corroboration across techniques is worth more than any single flagged region on its own.
...
Maps directly to [Process Injection (T1055)](https://attack.mitre.org/techniques/T1055/) — this lesson covers the artifact-level detection; [the next lesson](#/lesson/l3-06-injection-techniques) covers the specific techniques (process hollowing, DLL injection, and others) that produce what `malfind` is looking for.

> [!TIP]
> This lesson deliberately stopped at *recognizing* injected code, not *how it got there*. [The next lesson](#/lesson/l3-06-injection-techniques) is the technique catalog — process hollowing, DLL injection, and the other major methods, each with its own slightly different memory signature.
