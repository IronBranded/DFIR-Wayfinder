...
| 3 | Network Connection | Source/destination, process — the endpoint-side complement to [NetFlow](#/lesson/l6-03-netflow-analysis) |
| 6 | Driver Loaded | The primary [BYOVD detection surface](#/lesson/l5-15-byovd-loldrivers) |
| 7 | Image Loaded | DLL loads — relevant to [DLL hijacking](#/lesson/l5-05-com-dll-hijacking) |
| 8 | CreateRemoteThread | A core [process injection](#/lesson/l3-06-injection-techniques) indicator |
| 10 | Process Access | Captures [LSASS access](#/lesson/l3-09-lsass-memory-analysis) attempts directly |
| 11 | File Create | New file creation, useful for staging/dropper detection |
| 17 / 18 | Pipe Created / Connected | Named pipe activity — a [C2 framework fingerprinting](#/lesson/l6-04-c2-framework-fingerprinting) signal |
| 22 | DNS Query | Process-level DNS resolution, complementing [DNS log analysis](#/lesson/l6-01-dns-analysis) |
...
- **Sysmon service stopped or the driver unloaded** on a host where it's supposed to be running continuously — often a deliberate step in an intrusion specifically to blind logging before further activity (see the overlap with [BYOVD](#/lesson/l5-15-byovd-loldrivers), which can be used for exactly this).
