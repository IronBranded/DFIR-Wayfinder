...
- `lsass.exe` with a child process — [any credential dumping tool](#/lesson/l3-09-lsass-memory-analysis) triggers this, regardless of which one
- `w3wp.exe` spawning a command interpreter — [every web shell](#/lesson/l8-03-playbook-web-shell), regardless of implementation
- A `$SI` timestamp earlier than its `$FN` — [every timestomping tool](#/lesson/l4-05-mft-timestomping) that uses the standard API
- A replication request from a non-DC account — [every DCSync implementation](#/lesson/l7-06-dcsync-detection)
...
- [Amcache SHA-1](#/lesson/l4-02-amcache) — hash level. Excellent for pivoting, trivially defeated by recompilation.
- [C2 infrastructure from memory](#/lesson/l3-10-network-memory-artifacts) — IP/domain level. Perishable but immediately actionable.
- [Mutex names](#/lesson/l3-08-mutex-analysis) — host artifact level. More durable than a hash precisely because there is rarely a reason to vary them.
- [Process tree anomalies](#/lesson/l3-04-process-trees) — TTP level. The most durable detections in this academy.
