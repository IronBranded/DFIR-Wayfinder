[The previous lesson](#/lesson/l2-01-email-headers-authentication) covered the headers. This one covers the body: how a message is actually assembled, what the attachment is without opening it, and the mismatch between what a message displays and what it contains.
...
- **Container formats** — `.iso`, `.img`, `.vhd`. These historically did not propagate [Mark of the Web](#/lesson/l5-17-alternate-data-streams) to their contents, stripping the mark that causes Office to block macros. That property drove a real shift in delivery tradecraft.
