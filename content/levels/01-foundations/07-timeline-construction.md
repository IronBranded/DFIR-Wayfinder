...
- **`$MFT`** — MACB timestamps for every file ([with the `$SI`/`$FN` caveats](#/lesson/l2-02-mft-timestomping))
- **[USN journal](#/lesson/l2-03-usn-journal)** — file change events with reason codes
- **[Event logs](#/lesson/l2-09-event-log-key-ids)** — logons, process creation, service installs
- **Registry key `LastWriteTime`** — [persistence installation dates](#/lesson/l3-05-registry-run-keys)
- **[Prefetch](#/lesson/l2-01-prefetch)** — up to eight execution times per binary
