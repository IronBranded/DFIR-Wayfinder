...
The dot notation is a hierarchy, not a version number. `T1055.012` is one specific way of doing `T1055`, which is why [the injection techniques lesson](#/lesson/l5-05-injection-techniques) lists several sub-technique IDs under a single parent.
...
- **Coverage mapping** — identifying which tactics your telemetry genuinely covers versus assumes, which is what [the coverage map lesson](#/lesson/l1-09-attack-coverage-map) builds directly.
