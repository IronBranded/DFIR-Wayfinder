...
> [!TIP]
> For very small files there is a third option that beats both. Files under roughly 700 bytes are stored **resident**, inside the MFT record itself rather than in separate clusters — so [the `$MFT`](#/lesson/l4-05-mft-timestomping) may still contain the actual contents of a deleted script or configuration file, complete and with its metadata intact.
...
- **Deleted logs** — complementary to [carving EVTX records](#/lesson/l5-18-log-artifact-recovery) specifically.
