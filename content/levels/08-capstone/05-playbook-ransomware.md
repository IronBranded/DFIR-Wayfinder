...
**1. Isolate from the network — carefully.** Disconnect affected hosts from the network to stop lateral encryption. Resist powering off where you can: [memory holds evidence that disappears with power](#/lesson/l3-01-acquisition), sometimes including keys and always including the process and network state that explains what happened. Network isolation stops the spread; power-off destroys evidence.
