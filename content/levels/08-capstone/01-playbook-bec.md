...
**1. Contain the session, not just the password.** Reset the password *and* revoke sessions via PowerShell — a GUI-only reset leaves existing refresh tokens valid, and the attacker keeps working through them. If the account is hybrid, the password must be reset twice. The full mechanism is in [the hybrid runbook](#/lesson/l2-04-hybrid-runbook); this is the single most commonly botched step in BEC response.
...
- **What did they leave behind?** [Mailbox rules](#/lesson/l5-11-mailbox-forwarding-rules) — including hidden rules created via MAPI that don't render in Outlook — and forwarding configured at the mailbox or transport level.
- **What did they authorize?** [OAuth consent grants](#/lesson/l2-03-oauth-consent-grants) survive password resets entirely and are a frequently-missed persistence path.
