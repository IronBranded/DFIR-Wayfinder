...
**1. Get the original, not a forward.** A forwarded copy loses the original headers, and headers are where the [authentication results and routing path](#/lesson/l2-01-email-headers-authentication) live. Ask for it as an attachment, or pull it from the mailbox directly.
...
- **Anyone submitted credentials.** Every submitter is now a potential [BEC case](#/lesson/l8-01-playbook-bec) and needs session revocation, not just a password reset.
- **Anyone executed a payload.** That host now needs endpoint triage — [process tree](#/lesson/l3-04-process-trees) and [PowerShell logging](#/lesson/l3-13-powershell-logging) review at minimum.
